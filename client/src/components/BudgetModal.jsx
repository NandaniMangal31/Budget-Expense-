...
export default function BudgetModal({ isOpen, targetInputs, setTargetInputs, onSave, onClose }) {
  ...
  // Convert string inputs safely to numbers for calculations
  ...
  // useEffect runs every time user types any amount in the modal
  useEffect(() => {
    if (totalBudget === 0 && categoriesSum > 0) {
      ...
      setValidationError(""); // Clear error if everything is fine
    }
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto box-border">
          ...
          {validationError && (
            <div className="bg-red-50 border border-red-200 text-red-700 p-3.5 rounded-xl text-xs font-semibold leading-relaxed animate-shake">
              {validationError}
            </div>
          )}

          {/* Overall Total Budget Input */}
          <div>
          ...
          {["Food & Drinks", "Travel & Transport", "Shopping", "Bills & Utilities", "Entertainment", "Other"].map((cat) => (
            <div key={cat} className="flex items-center justify-between gap-3">
              ...
              <input 
                ...
                disabled={totalBudget === 0} // Blocks typing if overall budget is empty
              />
            </div>
          ))}
        </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
          ...
          <button 
            ...
            className={`px-5 py-2 rounded-lg text-xs font-bold text-white border-none transition-all cursor-pointer ${
              !!validationError || totalBudget === 0 ? "bg-slate-300 cursor-not-allowed shadow-none" : "bg-blue-600 hover:bg-blue-700 shadow-sm shadow-blue-200"
            }`}
          >
            Lock Target Config 🏆
          </button>
